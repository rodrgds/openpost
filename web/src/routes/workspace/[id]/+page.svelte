<script lang="ts">
	...
	onMount(async () => {
		...
		try {
			posts = await api.listPosts(workspaceId);
		} catch (e) {
...
<div class="mx-auto w-full max-w-[1360px] px-4 py-6 lg:px-8">
	...
	{:else if error}
		<div class="bg-destructive/10 border border-destructive/20 rounded-md p-4 text-destructive">
			<p>Error: {error}</p>
			<Button variant="ghost" size="sm" onclick={() => { error = ''; isLoading = true; }}>Retry</Button>
		</div>
			...
			<div class="overflow-x-auto">
				<table class="min-w-full divide-y divide-border">
					<thead class="bg-muted/50">
						<tr>
							<th class="px-6 py-3 text-left text-xs font-medium text-muted-foreground uppercase tracking-wider">Content</th>
							<th class="px-6 py-3 text-left text-xs font-medium text-muted-foreground uppercase tracking-wider">Status</th>
							<th class="px-6 py-3 text-left text-xs font-medium text-muted-foreground uppercase tracking-wider">Scheduled</th>
					...
					<tbody class="bg-background divide-y divide-border">
						{#each posts as post}
							<tr class="hover:bg-muted/50">
								<td class="px-6 py-4">
									<div class="text-sm max-w-md truncate">{post.content}</div>
								</td>
								<td class="px-6 py-4">
									<span class="px-2 py-1 text-xs font-medium rounded-full {getStatusColor(post.status)}">
										{post.status}
									</span>
								</td>
